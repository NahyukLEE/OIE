...
def intrinsic_loss(mask, pred_a, pred_s, gt_a, gt_s, gt_i):
    albedo_loss = gradient_loss(mask, gt_a, pred_a) + scale_invariant_loss(mask, gt_a, pred_a)
    shading_loss = gradient_loss(mask, gt_s, pred_s) + scale_invariant_loss(mask, gt_s, pred_s)
